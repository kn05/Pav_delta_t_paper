    ...
    derived_params=[
        (
            ...
            genn_model.create_dpf_class(
                lambda pars, dt: 1.0 / -((1.0 / pars[2]) + (1.0 / pars[3]))
            )(),
        )
    ],
...
def get_params(
    size_scale_factor=1, build_model=True, measure_timing=False, use_genn_recording=True
):
...
def plot(
    ...
    first_rewarded_stimuli_time_start = next(
        s[0] for s in start_stimulus_times if ((s[1], s[2], s[3]) == (0, 1, 2))
    )
    first_rewarded_stimuli_time_end = next(
        s[0] for s in end_stimulus_times if ((s[1], s[2], s[3]) == (0, 1, 2))
    )

    # Find the corresponding reward
    corresponding_reward_time_start = next(
        r for r in start_reward_times if r > first_rewarded_stimuli_time_start
    )
    corresponding_reward_time_end = next(
        r for r in end_reward_times if r > first_rewarded_stimuli_time_end
    )

    padding_start = (
        display_time_ms
        - (corresponding_reward_time_start - first_rewarded_stimuli_time_start)
    ) / 2
    padding_end = (
        display_time_ms
        - (corresponding_reward_time_end - first_rewarded_stimuli_time_end)
    ) / 2

    # Make spike sdf
    start_all_spikes = np.hstack([start_exc_spikes, start_inh_spikes])
    end_all_spikes = np.hstack([end_exc_spikes, end_inh_spikes])

    start_sdf = make_sdf(
        start_all_spikes[0][
            np.where(
                (start_all_spikes[0] > 3 * params["sigma"])
                & (start_all_spikes[0] < params["record_time_ms"] - 3 * params["sigma"])
            )
        ],
        0,
        params["record_time_ms"],
        1,
        params["sigma"],
    )
    end_sdf = make_sdf(
        end_all_spikes[0][
            np.where(
                ...
                & (end_all_spikes[0] < params["duration_ms"] - 3 * params["sigma"])
            )
        ],
    ...
    axes[0].scatter(
        start_exc_spikes[0], start_exc_spikes[1], s=2, edgecolors="none", color="red"
    )
    axes[0].scatter(
        start_inh_spikes[0],
        start_inh_spikes[1] + params["num_excitatory"],
        s=2,
        edgecolors="none",
        color="blue",
    )
    ...
    # plot start spike sdf

    start_peaks_index, _ = find_peaks(start_sdf, height=4)
    axes[1].plot(np.arange(0.0, params["record_time_ms"], 1.0), start_sdf)
    axes[1].plot(start_peaks_index, start_sdf[start_peaks_index], "x")
    axes[1].set_xlim(
        first_rewarded_stimuli_time_start - padding_start,
        corresponding_reward_time_start + padding_start,
    )
    axes[1].set_ylim(0, 70)
    for i in start_peaks_index:
        if (i > first_rewarded_stimuli_time_start - padding_start) and (
            i < corresponding_reward_time_start + padding_start
        ):
            axes[1].text(
                i,
                start_sdf[i] + 1,
    ...
    axes[2].scatter(
        end_exc_spikes[0], end_exc_spikes[1], s=2, edgecolors="none", color="red"
    )
    axes[2].scatter(
        end_inh_spikes[0],
        end_inh_spikes[1] + params["num_excitatory"],
        s=2,
        edgecolors="none",
        color="blue",
    )
    ...
    end_peaks_index, _ = find_peaks(end_sdf, height=4)
    axes[3].plot(
        np.arange(
            params["duration_ms"] - params["record_time_ms"], params["duration_ms"], 1.0
        ),
        end_sdf,
    )
    axes[3].plot(
        end_peaks_index + params["duration_ms"] - params["record_time_ms"],
        end_sdf[end_peaks_index],
        "x",
    )
    axes[3].set_xlim(
        first_rewarded_stimuli_time_end - padding_end + display_time_ms / 4,
        corresponding_reward_time_end + padding_end + display_time_ms / 4,
    )
    axes[3].set_ylim(0, 70)
    for i in end_peaks_index:
        if (
            i + params["duration_ms"] - params["record_time_ms"]
            > first_rewarded_stimuli_time_end - padding_end + display_time_ms / 4
        ) and (
            i + params["duration_ms"] - params["record_time_ms"]
            < corresponding_reward_time_end + padding_end + display_time_ms / 4
